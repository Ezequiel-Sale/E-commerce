...
const Cart: React.FC<IProduct> = () => {
  ...
  return (
    <>
      <h1 className="text-2xl text-center font-extrabold mt-24 m-2 md:flex md:justify-center text-white">
        Carrito de Compras
      </h1>
      <div className="flex flex-col items-center mt-4 mb-8 md:flex md:flex-row md:justify-between md:items-start">
        <div className="flex flex-wrap w-[70vw] gap-1 mb-16 justify-center md:flex md:justify-start md:flex-wrap md:ml-6 md:gap-3">
          {order?.length > 0 ? (
            order.map((orden: Product, indexCart) => {
              return (
                <div
                  key={indexCart}
            ...
            <div className="mb-8 flex flex-col items-center">
              <p className="text-3xl font-semibold mt-24 text-white">
                No hay productos en el carrito!
              </p>
              <Link href={"/"}>
                <Button className="bg-black mt-6 w-40 h-8">
                  Volver a productos
                </Button>
              </Link>
            </div>
        ...
        <div className="flex flex-col md:mr-28 md:mt-6">
          ...
          <p className="text-2xl text-center mt-8 font-semibold text-blue-500">
            ${order?.reduce((acc, curr) => acc + curr.price, 0)}
          </p>
          <Button className="bg-black mt-14 w-24 h-8 hover:bg-white hover:text-black" onClick={handleButton}>
            Comprar
          </Button>
        </div>
      </div>
